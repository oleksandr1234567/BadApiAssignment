...
export default class FetchAccessories extends React.Component {
  ...
  // Initial function to fetch the Accessories 
  FetchAccessories() {
    // Where the data is fetched from
    fetch(Url+"/products/accessories/")
      ...
      .then(data =>{
        ...
        this.setState(() => {
        return {          
          FetchAccessories: data,
          error: null,
          };
        },
        this.FetchAccessoriesAvailability
        )
      })
  ...
  checkLoadStatus (){
    //if(this.state.FetchCounter === this.state.FetchAccessories.length){
  ...
  //Function check the availability of the Accessories
  ...
  render() {
     ...
     const Row = ({ index, style }) => {
      ...
      let availability = this.availability[this.state.FetchAccessories[index].manufacturer].find(
        product => product.id.toLowerCase() === this.state.FetchAccessories[index].id.toLowerCase() 
        ).DATAPAYLOAD;
          ...
          <div style={style}>
            ...
            <div className="eachListRow">
              <h4> {this.state.FetchAccessories[index].name}  -   {this.state.FetchAccessories[index].color}  :  {this.state.FetchAccessories[index].price} € </h4>    
                <p>[ {this.state.FetchAccessories[index].manufacturer.toUpperCase()} ]: {availability.indexOf("OUTOFSTOCK") === -1 ? " In stock": " Out of stock"} <br />
                  id: {this.state.FetchAccessories[index].id} 
                </p>  
              <hr/> 
          </div>
          </div>
      ...
      return (
        <FixedSizeList
          height={500}
          width={"98%"}
          itemCount={this.state.FetchAccessories.length}
          itemSize={110}
          overscanCount={5}
          useIsScrolling={true}
        >
    ...
    if(!this.state.errorMessage){
            return (
              <React.Fragment  >
                ...
                //Render the list of Accessories and availability
